...
1. **Math, independently.** Recompute every extension (qty × unit, waste on material only),
   ...
   `python3 resources/validate_estimate.py <project_dir>/ --sector <sector>`
   (Resolve `resources/` against THIS SKILL's own folder, never your working
  directory — the copy always travels with the skill, on every surface.) — every FAIL is at least a Major finding;
