...
Copies of the 6 template skeletons the chat specialists fill in.
...
In chat you produce the two CSVs below; the user then hands them to **Claude Code**
(with the construction-estimating plugin) to build the formula-driven `estimate.xlsx`
and `estimate-summary.md`. Keeping the source as CSV makes the estimate diff-able and
lets the auditor recompute everything independently.
...
To build: hand `lineitems.csv` + `markups.csv` to Claude Code with the plugin
installed and ask it to *"build the estimate workbook"* — no local paths to remember.
