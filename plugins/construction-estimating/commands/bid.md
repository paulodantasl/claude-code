...
`estimating-projects/<project-slug>/`. When delegating to the subagents, pass each one the
absolute project path AND the `${CLAUDE_PLUGIN_ROOT}` location so they can read the knowledge base.
