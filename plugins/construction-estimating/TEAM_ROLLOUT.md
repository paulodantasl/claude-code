...
Cowork or claude.ai. Don't zip them by hand — the repo ships a validating packager:

```
python3 estimating/sync.py --write        # refresh each skill's bundled copies first
python3 estimating/package_skills.py      # validates, then writes dist/skills/*.zip
```

Upload one zip per skill via **Settings → Capabilities → Skills**. Two honest limits:
