...
def _commands(api_factory: Callable[[], IdealAPIs]) -> list[CommandSpec]:
    ...
    return [
        ...
        CommandSpec("permits", "fl-datasets", "Find Florida permit open-data sets", 1, False,
                    "ideal-api call permits fl-datasets --county Pinellas",
                    lambda **kw: api().permits.find_florida_permit_datasets(kw.get("county"))),
        CommandSpec("permits", "catalog", "Search Socrata open-data catalog (free)", 1, False,
                    'ideal-api call permits catalog --query "building permits Tampa"',
                    lambda **kw: api().permits.socrata_catalog_search(kw["query"], limit=int(kw.get("limit", 20)))),
        # Tier 1 — weather
        ...
        CommandSpec("weather", "alerts", "Active NWS weather alerts", 1, False,
                    "ideal-api weather alerts --state FL --zone FLZ151",
                    lambda **kw: api().weather.nws_alerts(
                        state=kw.get("state"), zone=kw.get("zone"),
                    )),
        ...
        CommandSpec("weather", "stormglass", "Coastal marine weather", 1, True,
                    "ideal-api call weather stormglass --lat 27.77 --lon -82.63",
                    lambda **kw: api().weather.stormglass_weather(float(kw["lat"]), float(kw["lon"]))),
        CommandSpec("weather", "visual-crossing", "Weather timeline + events (Visual Crossing)", 1, True,
                    'ideal-api call weather visual-crossing --location "Tampa,FL"',
                    lambda **kw: api().weather.visual_crossing_timeline(kw["location"])),
        # Tier 1 — government
        ...
        CommandSpec("gov", "census-geocode", "Census geocoder (free)", 1, False,
                    'ideal-api call gov census-geocode --address "401 E Jackson St, Tampa FL"',
                    lambda **kw: api().government.census_geocoder(kw["address"])),
        CommandSpec("gov", "openfema", "Recent FEMA disaster declarations", 1, False,
                    "ideal-api call gov openfema --states FL --days 90",
                    lambda **kw: api().government.openfema_disasters(
                        states=[s.strip() for s in kw.get("states", "FL").split(",") if s.strip()],
                        days_back=int(kw.get("days", kw.get("days_back", 90))),
                        limit=int(kw.get("limit", 25)),
                    )),
        CommandSpec("gov", "epa", "EPA Envirofacts query", 1, False,
                    "ideal-api call gov epa --table frs_program_facility/state/FL/1:5",
                    lambda **kw: api().government.epa_envirofacts(kw["table"], rows=int(kw.get("rows", 5)))),
        # Tier 1 — web
        ...
        CommandSpec("docs", "pandadoc-templates", "List PandaDoc templates", 2, True,
                    "ideal-api call docs pandadoc-templates",
                    lambda **kw: api().documents.pandadoc_list_templates()),
        CommandSpec("docs", "buildpdf", "HTML to PDF (BuildPDF)", 2, True,
                    'ideal-api call docs buildpdf --html "<h1>Proposal</h1>"',
                    lambda **kw: api().documents.buildpdf(kw["html"])),
        # Tier 2 — geo
        ...
        CommandSpec("logistics", "track", "Track a parcel (WhereParcel)", 2, True,
                    "ideal-api call logistics track --number 1Z999AA10123456784",
                    lambda **kw: api().logistics.whereparcel_track(kw["number"], carrier=kw.get("carrier"))),
        CommandSpec("logistics", "ups-track", "Track a parcel (UPS OAuth)", 2, True,
                    "ideal-api call logistics ups-track --number 1Z999AA10123456784",
                    lambda **kw: api().logistics.ups_track(kw["number"])),
        # Tier 2 — property
        ...
        CommandSpec("productivity", "clockify-workspaces", "List Clockify workspaces", 2, True,
                    "ideal-api call productivity clockify-workspaces",
                    lambda **kw: api().productivity.clockify_workspaces()),
        CommandSpec("productivity", "clockify-projects", "List Clockify projects in a workspace", 2, True,
                    "ideal-api call productivity clockify-projects --workspace-id ID",
                    lambda **kw: api().productivity.clockify_projects(kw["workspace_id"])),
    ]
