...
- **State after run: 56 parameters** (16 area, 16 linear, 24 count) spanning divisions 00/02/03/06/07/08/09/10/12/21/22/23/26/27/28, each name carrying its basis, tolerance or open RFI. **Open next:** geometry-anchored annotations on A1/A2/A6 (already calibrated — the ideal precondition), and cost-item wiring (`createCostItem` + quantity formulas referencing these parameter names).
